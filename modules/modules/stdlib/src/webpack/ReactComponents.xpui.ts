...
const [NavigationModule] = modules.find(
	([_, v]) =>
		fnStr(v).includes("navigationalRoot") && fnStr(v).includes("noLink"),
);
...
const [ContextMenuModuleID] = modules.find(([_, v]) =>
	fnStr(v).includes("toggleContextMenu"),
)!;
export const ContextMenu: any = Object.values(
	webpackRequire(ContextMenuModuleID),
)[0];
...
export const Tooltip: React.FC<any> = findBy(
	"hover-or-focus",
	"tooltip",
)(exportedFunctions);

export const Menu: React.FC<any> = findBy(
	"getInitialFocusElement",
	"children",
)(exportedFunctions);

export const MenuItem: React.FC<any> = findBy(
	"handleMouseEnter",
	"onClick",
)(exportedFunctions);

export const MenuItemSubMenu: React.FC<any> =
	findBy("subMenuIcon")(exportedFunctions);
...
const [ScrollableContainerModule] = modules.find(
	([_, v]) =>
		fnStr(v).includes("scrollLeft") && fnStr(v).includes("showButtons"),
);
const ScrollableContainerExports = Object.values(
	webpackRequire(ScrollableContainerModule),
);
export const ScrollableContainer: React.FC<any> =
	ScrollableContainerExports.find(
...
export const Router: React.FC<any> = findBy(
	"navigationType",
	"static",
)(exportedFunctions);
...
export const Dialog: React.FC<any> = findBy(
	"isOpen",
	"unmountWhenClose",
)(exportedFunctions);
...
export const IconWrapper: React.FC<any> = findBy("button__icon-wrapper")(
	exportedFunctions,
);
