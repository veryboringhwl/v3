...
const sendProtocolMessage = async (
	action: string,
	...
	if (!daemonConn) {
		await tryConnectToDaemon();
	}

	if (daemonConn && daemonConn.readyState === WebSocket.OPEN) {
		const listener = (e: any) => handleIncomingMessage(e.data);
		cancelSubscription = () =>
			daemonConn?.removeEventListener("message", listener);

		daemonConn.addEventListener("message", listener);
		daemonConn.send(uri);
	} else {
		return reject(new Error("Local Daemon not connected. Action aborted."));
	}
	setTimeout(() => {
