...
export class Module extends ModuleBase<Module, ModuleInstance> {
  ...
  public canEnable(instance: ModuleInstance) {
    ...
    return instance.canDelete() && (!enabledInstance || this.canDisable(enabledInstance));
  }
...
export type ContextPromise = PromiseWithResolvers<DisposeFn | undefined> & {
  wrap: (promise: Promise<DisposeFn | undefined>) => Promise<DisposeFn | undefined>;
};

function createContextPromise(): ContextPromise {
  const promise = Promise.withResolvers<DisposeFn | undefined>();
...
export const hotwire = <C extends {}>(
  ...
  return async <R extends undefined | DisposeFn>(ctx: C): Promise<R> => {
