...
) => SyncOrAsync<void> | PromiseLike<DisposeFn | void>;
...
export class Module extends ModuleBase<Module, ModuleInstance> {
	...
	public async enable(instance: ModuleInstance): Promise<boolean> {
		const result = await instance.transition.new(async () => {
	...
	public async disable(): Promise<boolean> {
		...
		const result = await enabledInstance.transition.new(async () => {
...
export class ModuleInstance
	extends ModuleInstanceBase<Module>
	implements MixinLoader
{
	...
	public async loadProviders() {
		...
		const vault = await fetchJson<_Vault>(this.getRelPath("vault.json")!).catch(
			() => null,
		);
	...
	public awaitedMixins = [] as Promise<void>[];
	...
	async #loadCssIndex() {
		...
		this._cssIndex = Object.assign(
			{},
			await import(uniqueEntry, { with: { type: "css" } }),
			{
				disposableStack: new AsyncDisposableStack(),
			},
		);
	...
	public isEnabled() {
		return (
			this.isInstalled() && // just a safeguard
...
const getLoadableChildrenInstances = () =>
	Array.from(RootModule.INSTANCE.getDescendantsByBreadth())
		...
		.filter((instance) => instance?.canLoad()) as ModuleInstance[];
...
export type ContextPromise = PromiseWithResolvers<DisposeFn | void> & {
...
declare global {
	var __HOTWIRED__: Record<
		string,
		PromiseWithResolvers<ContextWithPromise<any>>
	>;
	var __HOTWIRED_ABORT__: Record<string, AbortController>;
}

globalThis["__HOTWIRED__"] ??= {};
globalThis["__HOTWIRED_ABORT__"] ??= {};
...
export const hotwire = <C extends {}>(
	...
	globalThis["__HOTWIRED_ABORT__"][nurl] = controller;
...
export const hotwired = <C extends {}>(
	meta: ImportMeta,
): Promise<ContextWithPromise<C>> => {
...
function normalizeUrl(url: string, base?: string, raw = false) {
	let nurl = base ? new URL(url, base).href : import.meta.resolve(url);
