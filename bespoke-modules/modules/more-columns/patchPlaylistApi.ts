...
export async function patchPlaylistContents(contents: any, opts: any) {
  const items = (contents.items as any[]).filter(
    (track) => is.Track(track.uri) && !is.LocalTrack(track.uri),
  );
  ...
  const needsAlbumData =
    CONFIG.enablePlaycount || CONFIG.enableReleaseDate || isPlaycountSort || isReleaseDateSort;
  ...
  const limit = opts?.limit ?? (opts?.offset === undefined ? 0 : contents.items.length);
