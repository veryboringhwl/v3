...
const KeybindKey = ({ children }: { children: string }) => {
  return (
    <UI.Text as="kbd" className="hykQHtPI6EeFREwqRrOR" variant="bodyMedium">
      {parseKey(children)}
    </UI.Text>
  );
};
        ...
        combinations.map((combination, i) => {
          ...
          return (
            <React.Fragment key={combination}>
              {keys.map((key) => (
                <KeybindKey key={key}>{key}</KeybindKey>
              ))}
              {i < combinations.length - 1 && (
                <UI.Text variant="bodyMedium"> {Locale.get("keyboard.shortcuts.or")} </UI.Text>
              )}
            </React.Fragment>
          );
        })
...
const KeybindSection = ({
  ...
  return (
    <>
      <UI.Text as="h2" className="ARw2f2PkF29n9Ek_eWu3" variant="titleSmall">
        {sectionTitle}
      </UI.Text>
      <ul>
        {shortcuts.map((shortcut) => {
          ...
          return (
            <li className="umavpIt6VOGqirdlUYWs" key={shortcut.action}>
              <UI.Text className="dYBZmh_ZIyvBZfaoducd" variant="bodyMedium">
                {shortcut.description}
              </UI.Text>
              <KeybindCombinations action={shortcut.action} />
            </li>
          );
        })}
      </ul>
