  ...
  implements Map<ModuleIdentifier, ReadonlySet<ModuleInstance>>
{
  ...
  accumulate(...instances: ModuleInstance[]) {
    const instancesByModuleIdentifier = Object.groupBy(instances, (instance) =>
      instance.getModuleIdentifier(),
    );
...
export type SharedIntanceGeneratorFactories = WeakMap<
  ModuleInstance,
  AsyncGeneratorFactory<DependencyTree>
>;
...
export function flattenDTrees(dependencyTrees: DependencyTree | DependencyTree[]) {
...
async function* getModuleInstances(
  ...
  const versions = Array.from(module?.instances.keys() ?? []).filter((version) =>
    satisfies(parse(version), parseRange(versionRange)),
  );
...
export async function* getInstanceDTreeCandidates(
  ...
  async function* _getInstanceDTreeCandidates(
    ...
    for await (const candidate of getDependenciesDTreeCandidates(
      instance.metadata?.dependencies,
      accumulator,
      sigfs,
    )) {
...
export async function* getDependenciesDTreeCandidates(
  ...
  const instanceGens = Object.entries(dependencies).map(([moduleIdentifier, versionRange]) =>
    getModuleInstances(moduleIdentifier, versionRange),
  );
...
function createSharedGeneratorFactory<T>(
  generator: AsyncGenerator<T>,
): AsyncGeneratorFactory<Awaited<T>> {
  const cache: T[] = [];
