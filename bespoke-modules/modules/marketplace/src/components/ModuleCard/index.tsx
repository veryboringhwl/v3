import type { Metadata, Module, ModuleIdentifier, ModuleInstance } from "/hooks/module.ts";
...
export const useManageModules = (props: useManageModulesProps) => {
  const fastRemove = async (moduleInstance: ModuleInstance) => {
    ...
    Snackbar.enqueueSnackbar(`Failed to remove ${moduleInstance.getIdentifier()}`, {
      variant: "error",
    });
    return false;
  };
  ...
  const fastEnable = async (moduleInstance: ModuleInstance) => {
    ...
    Snackbar.enqueueSnackbar(`Failed to enable ${moduleInstance.getIdentifier()}`, {
      variant: "error",
    });
    return false;
  };
  ...
  const fastEnableWithDependencies = async (...moduleInstances: ModuleInstance[]) => {
    ...
    const moduleIdentifiers = moduleInstances
      .map((instance) => instance.getIdentifier())
      .join(", ");
...
const ModuleCard = (props: ModuleCardProps) => {
  ...
  const fastInstallDeleteButton = isInstalled ? (
    <>
      <MdDeleteForever className="w-4 h-4" title="Disable, Delete, and Remove" />
      Remove
    </>
  ) : (
    <>
      ...
      >
        ...
        <div className="flex flex-col gap-2 flex-grow">
          ...
          <div className="text-[var(--text-subdued)] whitespace-normal main-type-mestoBold">
            <TagsDiv importantTags={importantTags} showTags={showTags} tags={tags} />
          </div>
