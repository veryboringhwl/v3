...
const _useModules = () => {
  ...
  const setModulesForIdentifier = React.useCallback(
    (identifier: ModuleIdentifier, f: (_modules: [Module]) => [Module]) => {
      setModules((modules) => {
  ...
  for (const [identifier, moduleInstance] of Object.entries(defaultModuleToInstance)) {
    moduleToInstance[identifier] = moduleToInstanceRef.current[identifier] ?? moduleInstance;
  }
  ...
  const [selectedModules, selectModules] = React.useState<ModuleIdentifier[]>([]);
