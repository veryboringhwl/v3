...
import { fetchArtistLikedTracks } from "/modules/Delusoire.delulib/lib/platform.ts";
...
export const getTracksFromArtist = async (uri: string) => {
  ...
  const itemsWithCountAr: ItemsWithCount<ItemMin>[] = [];
  const itemsReleasesAr: ItemsReleases<ItemMin>[] = [];
  const appearsOnAr: ItemsReleasesWithCount<ItemMin>[] = [];
  ...
  if (CONFIG.artistAllDiscography) {
    ...
    CONFIG.artistTopTracks &&
      allTracks.push(...discography.topTracks.items.map(parseTopTrackFromArtist));
  ...
  const appearsOnUris = appearsOnAr
    .flatMap((ir) => ir.items.flatMap((i) => i.releases.items))
    .map((item) => item.uri);
