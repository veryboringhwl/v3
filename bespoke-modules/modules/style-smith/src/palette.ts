...
export class Theme implements Serializable<SerializedThemeData> {
    ...
    return Object.entries(themes)
      .map(([type, theme]) => {
        return Object.entries(theme)
          .map(([set, scheme]) => colorSetSchemeToCss(type as ThemeType, set as ColorSet, scheme))
          .join("\n\n")
          .replaceAll(/\n[^\S\n]+\n/g, "\n\n");
      })
      .join("\n\n\n\n")
  ...
  toJSON(): SerializedThemeData {
    return mapValues(this.theme, (scheme) =>
      nestColorScheme(
  ...
  static fromJSON(json: SerializedThemeData) {
    const theme = mapValues(json, (scheme) =>
      mapValues(flattenColorScheme(scheme), (colorPair) =>
        mapValues(colorPair, (color) => Color.parse(JSON.stringify(color)) as Color),
      ),
    );
  ...
  static createDefault() {
    ...
    for (const [type, theme] of Object.entries(defaultThemes)) {
      for (const [set, scheme] of Object.entries(theme)) {
        themes[type as ThemeType][set as ColorSet] = mapValues(scheme, (color) => ({
          [type]: Color.fromCSS(color) as Color,
