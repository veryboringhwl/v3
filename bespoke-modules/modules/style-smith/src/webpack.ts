...
const [PaletteModuleID] = modules.find(([, m]) =>
  m.toString().includes('"Invalid hexadecimal color input"'),
)!;
const PaletteModule = webpackRequire(PaletteModuleID);
const PaletteExports = Object.values(PaletteModule);

export const appliedColorTheme = PaletteExports.find(
  (e: any) => e?.base?.background?.base,
) as ColorTheme;
...
export const colorSetSchemeToCss = (
  ...
  return [
    ...
    "\n" +
      ...
      " {\n" +
      "\n      --parents-essential-base: " +
      scheme.essential.base +
      ";\n    " +
      "\n}\n",
    "",
  ].join("\n");
...
async function getDefaultColorThemes() {
  ...
  for (const match of css.matchAll(R)) {
    ...
    const entries = attributes.split(";").map((attr) => {
      ...
      return [
        toCamelCase(name) as keyof FlatColorScheme,
        value.trim().replaceAll(/(?<!\d)\./g, "0."),
      ];
    });
