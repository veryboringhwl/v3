...
const vertShader = await fetch(
  "/modules/Delusoire.better-lyrics/amll/bg-render/shaders/base.vert.glsl",
).then((res) => res.text());
const _fragShader = await fetch(
  "/modules/Delusoire.better-lyrics/amll/bg-render/shaders/base.frag.glsl",
).then((res) => res.text());
const blendShader = await fetch(
  "/modules/Delusoire.better-lyrics/amll/bg-render/shaders/blend.frag.glsl",
).then((res) => res.text());
const eplorShader = await fetch(
  "/modules/Delusoire.better-lyrics/amll/bg-render/shaders/eplor.frag.glsl",
).then((res) => res.text());
const noiseShader = await fetch(
  "/modules/Delusoire.better-lyrics/amll/bg-render/shaders/noise.frag.glsl",
).then((res) => res.text());
...
function _brightnessImage(imageData: ImageData, brightness: number) {
  const pixels = imageData.data;
...
class GLBuffer implements Disposable {
  ...
  private type: number;
  constructor(gl: WebGL2RenderingContext, type: number, data: ArrayBufferView, usage: number) {
    this.gl = gl;
    this.type = type;
    this.data = data;
    this.usage = usage;
...
export class EplorRenderer extends BaseRenderer {
  ...
  private mainProgram: GLProgram = new GLProgram(this.gl, vertShader, eplorShader, "main");
  private blendProgram: GLProgram = new GLProgram(this.gl, vertShader, blendShader, "blend");
  private noiseProgram: GLProgram = new GLProgram(this.gl, vertShader, noiseShader, "noise");

  private static readonly rawVertexBuffer = new Float32Array([-1, -1, 1, -1, -1, 1, 1, 1]);
  ...
  constructor(protected canvas: HTMLCanvasElement) {
    ...
    this.fb = [new Framebuffer(this.gl, width, height), new Framebuffer(this.gl, width, height)];
  ...
  protected override onResize(width: number, height: number): void {
    ...
    this._targetSize = [Math.max(1, Math.round(width)), Math.max(1, Math.round(height))];
