...
import type { Disposable, HasElement, LyricLine, LyricWord } from "../interfaces.ts";
import { createMatrix4, matrix4ToCSS, scaleMatrix4 } from "../utils/matrix.ts";
import { Spring } from "../utils/spring.ts";
import type { LyricPlayer } from "./index.ts";
...
const _defaultEmpEasing = makeEmpEasing(EMP_EASING_MID);
...
function chunkAndSplitLyricWords(words: LyricWord[]): (LyricWord | LyricWord[])[] {
...
export class LyricLineEl extends EventTarget implements HasElement, Disposable {
      ...
      for (const a of word.maskAnimations) {
        if (this.lyricAdvanceDynamicLyricTime) {
          ...
          if (maskAnimationTime - this.lyricLine.startTime <= 0) {
            ...
            !this.areWordsOnSameLine(this.splittedWords[i - 1], this.splittedWords[i]) &&
            current < start - 300
          ) {
            a.currentTime = start;
            a.playbackRate = 1;
  ...
  measureSize(): [number, number] {
    ...
    const size: [number, number] = [this.element.clientWidth, this.element.clientHeight];
    if (this._hide) {
      if (this._prevParentEl) {
        this.element.remove();
      }
  ...
  rebuildStyle() {
    ...
    style += `transform:translate(${this.lineTransforms.posX
      ...
      .getCurrentPosition()
      .toFixed(1)}px) scale(${this.lineTransforms.scale.getCurrentPosition().toFixed(4)});`;
  ...
  private initEmphasizeAnimation(
    ...
    result = characterElements.flatMap((el, i, arr) => {
      ...
      const frames: Keyframe[] = new Array(ANIMATION_FRAME_QUANTITY).fill(0).map((_, j) => {
      ...
      const floatFrame: Keyframe[] = new Array(ANIMATION_FRAME_QUANTITY).fill(0).map((_, j) => {
        const x = (j + 1) / ANIMATION_FRAME_QUANTITY;
  ...
  setTransform(
    ...
    const roman = this.element.children[2] as HTMLDivElement;
    main.style.opacity = `${
      opacity * (!this.hasFaded ? 1 : this.lyricPlayer._getIsNonDynamic() ? 1 : 0.3)
    }`;
