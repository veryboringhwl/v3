...
import { LyricLineEl, type RawLyricLineMouseEvent, shouldEmphasize } from "./lyric-line.ts";
...
export class LyricPlayer extends EventTarget implements HasElement, Disposable {
  ...
  private resizeObserver: ResizeObserver = new ResizeObserver((e) => {
    ...
    const innerWidth =
      this.element.clientWidth - parseFloat(styles.paddingLeft) - parseFloat(styles.paddingRight);
    const innerHeight =
      this.element.clientHeight - parseFloat(styles.paddingTop) - parseFloat(styles.paddingBottom);
  ...
  setLyricLines(lines: LyricLine[], initialTime = 0) {
    ...
    this.processedLines = lines
      .filter((line) => line.words.reduce((pv, cv) => pv + cv.word.trim().length, 0) > 0)
      .map((line, i, lines) => {
        if (line.isBG) {
          return {
            ...line,
  ...
  calcLayout(force = false, reflow = false) {
    ...
    this.lyricLinesEl.forEach((el, i) => {
      ...
      if (
        ...
        ((i === this.scrollToIndex && interlude?.[2] === -2) || i === this.scrollToIndex + 1)
      ) {
        setDots = true;
