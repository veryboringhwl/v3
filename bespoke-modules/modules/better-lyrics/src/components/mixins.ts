...
import { html, type LitElement } from "https://esm.sh/lit";
...
export const SyncedMixin = <T extends Constructor<LitElement>>(superClass: T) => {
  class mixedClass extends superClass {
    ...
    updateProgress(_scaledProgress: number, _depthToActiveAncestor: number) {}
  }
...
export const ScrolledMixin = <T extends Constructor<LitElement & SyncedMixinI>>(superClass: T) => {
  class mixedClass extends superClass {
    ...
    override updateProgress(progress: number, depthToActiveAncestor: number) {
      ...
      const lineHeight = parseInt(document.defaultView?.getComputedStyle(this).lineHeight, 10);
...
export const SyncedContainerMixin = <T extends Constructor<LitElement & SyncedMixinI>>(
  superClass: T,
) => {
  class mixedClass extends superClass {
    ...
    computeChildProgress(rp: number, _child: number) {
      return rp;
    }
