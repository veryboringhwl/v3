...
export const findLyrics = async (info: {
  ...
  if (richSync) {
    ...
    const delimiters = slidingWindows([{ tep: 0 }, ...wordSynced, { tsp: 1 }], 2).map(
      ([prev, next]) => {
        return null;
...
const _getTranslation = async (trackId: string, lang = "en") => {
...
const fetchMxmMacroSubtitlesGet = async (
  ...
  return {
    ...
    subtitles:
      trackSubtitlesGet.message.body.subtitle_list?.map(
