...
export class KochanekBartels extends CubicHermite {
  ...
  static fromAlpha(
    ...
    const grid = deltas.reduce(
      (partialSums, delta) => [...partialSums, partialSums.at(-1)! + delta],
      [0],
    );
  ...
  private constructor(
    ...
    if (closed) {
      ...
      const tangent = scalarMultVector(
        grid[1] - grid[0],
        vectorSubVector(vertices[1], vertices[0]),
      );
...
function _end_tangent(
  ...
  return condition === EndCondition.NATURAL
    ? _natural_tangent(vertices, times, other_tangent)
    : condition;
}
