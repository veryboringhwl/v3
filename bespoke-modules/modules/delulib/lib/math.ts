...
export const vectorMultVector = (u: vector, v: vector) =>
  zip(u, v).map(([uix, vix]) => uix! * vix!);
export const vectorDotVector = (u: vector, v: vector) =>
  vectorMultVector(u, v).reduce((a, b) => a + b, 0);
...
export const vectorCartesianVector = (u: vector, v: vector) =>
  u.map((ux) => v.map((vx) => [ux, vx] as const));
