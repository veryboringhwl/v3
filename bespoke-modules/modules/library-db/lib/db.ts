...
import {
  chunkify,
  getConcurrentExecutionLimiterWrapper,
} from "/modules/Delusoire.delulib/lib/fp.ts";
...
const fetchOrPopulateDB =
  <A, B extends string>(table: Table<A, B>, fetcher: (primaryKeys: B[]) => Promise<A[]>) =>
  async (primaryKeys: B[]) => {
...
export const getPlaylistsFromURIs = fetchOrPopulateDB(db.playlists, (uris) =>
  Promise.all(uris.map((uri) => getPlaylist(uri))),
);
