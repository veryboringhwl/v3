...
globalThis.__patchTracklistWrapperProps = (props) => {
  ...
  p.renderRow = React.useCallback(
    (data: Data, index: number) => (
      <Row data={data} index={index} key={index} renderRow={props.renderRow} />
    ),
...
globalThis.__patchTracklistColumnHeaderContextMenu =
  (columnType) =>
  ({ className, children, onSort, columnIndex }: any) => {
    ...
    return React.createElement(
      ...
      <UI.Text
        className={classnames("standalone-ellipsis-one-line", className)}
        variant="bodySmall"
      >
        {column.label}
      </UI.Text>,
  ...
  if (!v[Symbol.for("patched")]) {
    Object.defineProperty(m, k, {
      ...
      get: () => (options: Options) => {
        ...
        return [
          ...columns.slice(0, i),
          ...Array.from(COLUMN_TYPES_PLAYLISTS).flatMap(([type, cond]) =>
            cond(options) ? [type] : [],
          ),
          ...columns.slice(i),
        ];
