import type { IndexLoadFn, IndexMixinFn, IndexPreloadFn } from "/hooks/module.ts";
...
export const preload: IndexPreloadFn = async () => {
  ...
  const [m, k, v] = exports.flatMap((m) => {
    ...
    for (const k of keys) {
      ...
      const s = v.toString();
      if (!/\(\{columnType:[a-zA-Z_$][\w$]*,visible:(?:[^,}]+),toggleable:(?:[^,}]+),/.test(s)) {
        continue;
      }
      return [[m, k, v]];
    }
  ...
  if (!v[Symbol.for("patched")]) {
    const patchedTracklistColumnsProvider = (props: any) =>
      v({
        ...props,
        columns: globalThis.__patchTracklistColumnsProvider(props.columns),
      });
...
export const load: IndexLoadFn = async (_context) => {
