...
import type { LFMTopAlbums } from "./types/LFMTopAlbums.ts";
import type { LFMTopArtists } from "./types/LFMTopArtists.ts";
import type { LFMTopTracks } from "./types/LFMTopTracks.ts";
...
export const fetchLFMTopTracks =
  (key: string) => (username: string, range: keyof typeof SpotifyTimeRange) => {
...
export const fetchLFMTopArtists =
  (key: string) => (username: string, range: keyof typeof SpotifyTimeRange) => {
    const url = new URL(LFMEndpoint);
