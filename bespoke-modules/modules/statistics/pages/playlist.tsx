...
export const fetchAudioFeaturesMeta = async (ids: string[]) => {
  const audioFeaturesLists = {
    danceability: [] as number[],
    energy: [] as number[],
    key: [] as number[],
    loudness: [] as number[],
    mode: [] as number[],
    speechiness: [] as number[],
    acousticness: [] as number[],
    instrumentalness: [] as number[],
    liveness: [] as number[],
    valence: [] as number[],
    tempo: [] as number[],
    time_signature: [] as number[],
  };
...
export const fetchAlbumsMeta = async (ids: string[]) => {
  const idToMult = mapValues(
    Object.groupBy(ids, (x) => x),
    (ids) => ids?.length,
  );
  ...
  const { status, error, data } = useQuery({
    ...
    queryFn: async () => {
      ...
      const duration = tracks
        .map((track) => track.duration.milliseconds as number)
        .reduce((a, b) => a + b, 0);
