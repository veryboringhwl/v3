...
const getServerCertificate = (() => {
  ...
  return async () => {
    ...
    const cacheControl = cert.headers.get("cache-control");
    if (cacheControl) {
...
const loadTrackIntoPlayer = (player: HTMLMediaElement) => async (trackUri: string) => {
  ...
  const fileId = await getTrackMeta(trackHex).then(
    (res: any) =>
...
const getSongPositionMs = (state = PlayerAPI.getState()) => {
  ...
  const { positionAsOfTimestamp, timestamp, duration, speed, hasContext, isPaused, isBuffering } =
    state;
...
class Bar {
  ...
  update(amplitude: number) {
    ...
    this.opacityScale =
      this.smoothing * oldOpacityScaleTarget + (1 - this.smoothing) * opacityScaleTarget;

    const oldHeightScaleTarget = 0.05 + 0.95 * this.normedAmp;
    const heightScaleTarget = 0.05 + 0.95 * normedAmp;
    this.heightScale =
      this.smoothing * oldHeightScaleTarget + (1 - this.smoothing) * heightScaleTarget;
