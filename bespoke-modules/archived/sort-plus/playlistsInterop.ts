...
export const createPlaylistFromLastSortedQueue = async () => {
  ...
  const sortedPlaylistsFolder = await fetchFolder(CONFIG.sortedPlaylistsFolderUri).catch(
    fetchRootFolder,
  );
...
export const reordedPlaylistLikeSortedQueue = async () => {
  ...
  const reversedPlaylistUids: string[] = (await fetchPlaylistContents(lastFetchedUri))
    .map((item) => item.uid)
    .reverse();

  let i = sortedUids.length - 1;
  const uidsByReqs: string[][] = [];
  while (i >= 0) {
    const uids: string[] = [];
