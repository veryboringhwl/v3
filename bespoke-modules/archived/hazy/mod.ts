...
const getPlayerAPIPref = (key: string) =>
  PlayerAPI._prefs.get({ key }).then((res) => res.entries[key]);

function fetchValues() {
  const parseIntFromStorage = (key: string) => Number.parseInt(localStorage.getItem(key), 10);
...
async function controlDimensions() {
  ...
  const isTouchscreenUi =
    enableGlobalNavBar === "home-next-to-navigation" ||
    enableGlobalNavBar === "home-next-to-search";
  ...
  if (isTouchscreenUi) {
    ...
    const panelGap = Number.parseInt(styleMap.get("--panel-gap")[0], 10);
  ...
  function lyricsCallback(mutationsList, lyricsObserver) {
    for (const mutation of mutationsList)
      for (const addedNode of mutation.addedNodes ?? [])
        if (addedNode.classList?.contains("kGR_hu4tdj9PnUlSPaRL")) setLyricsPageProperties();
